...
struct Board {
    cells: [[isize; SIZE]; SIZE],
}

impl Board {
    fn new() -> Self {
        Self {
            cells: [[UNVISITED; SIZE]; SIZE],
        }
    }
    ...
    fn valid_move(&self, pos: &Cell, r#move: &(isize, isize)) -> bool {
        if pos.x + r#move.0 > 7 || pos.x + r#move.0 < 0 {
            return false;
        }
        if pos.y + r#move.1 > 7 || pos.y + r#move.1 < 0 {
            return false;
        }
...
fn find_tour(board: &mut Board, cell: Cell, count: isize) -> bool {
    loop {
        ...
        println!("visited: {} on move {}", &cell, count);

        if let Some(next_cell) = board.next_move(cell) {
